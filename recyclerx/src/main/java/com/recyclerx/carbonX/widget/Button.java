...
import com.recyclerx.carbonX.CarbonX;
import com.recyclerx.carbonX.animation.AnimUtils;
import com.recyclerx.carbonX.animation.AnimatedColorStateList;
import com.recyclerx.carbonX.animation.AnimatedView;
import com.recyclerx.carbonX.animation.StateAnimator;
import com.recyclerx.carbonX.drawable.DefaultPrimaryColorStateList;
import com.recyclerx.carbonX.drawable.ripple.RippleDrawable;
import com.recyclerx.carbonX.drawable.ripple.RippleView;
import com.recyclerx.carbonX.internal.TypefaceUtils;
import com.recyclerx.carbonX.shadow.Shadow;
import com.recyclerx.carbonX.shadow.ShadowGenerator;
import com.recyclerx.carbonX.shadow.ShadowShape;
 ...
 */
public class Button extends AppCompatButton implements ShadowView, RippleView, TouchMarginView, StateAnimatorView, AnimatedView, CornerView, TintedView {
    ...
    public Button(Context context, AttributeSet attrs) {
        super(CarbonX.getThemedContext(context, attrs, R.styleable.ButtonX, android.R.attr.buttonStyle, R.styleable.ButtonX_carbonX_theme), attrs, android.R.attr.buttonStyle);
        initButton(attrs, android.R.attr.buttonStyle);
    }

    public Button(Context context, AttributeSet attrs, int defStyleAttr) {
        super(CarbonX.getThemedContext(context, attrs, R.styleable.ButtonX, defStyleAttr, R.styleable.ButtonX_carbonX_theme), attrs, defStyleAttr);
        initButton(attrs, defStyleAttr);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public Button(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(CarbonX.getThemedContext(context, attrs, R.styleable.ButtonX, defStyleAttr, R.styleable.ButtonX_carbonX_theme)/*, attrs, defStyleAttr, defStyleRes*/);
        initButton(attrs, defStyleAttr);
    }

    private static int[] rippleIds = new int[]{
            R.styleable.ButtonX_carbonX_rippleColor,
            R.styleable.ButtonX_carbonX_rippleStyle,
            R.styleable.ButtonX_carbonX_rippleHotspot,
            R.styleable.ButtonX_carbonX_rippleRadius
    };
    private static int[] animationIds = new int[]{
            R.styleable.ButtonX_carbonX_inAnimation,
            R.styleable.ButtonX_carbonX_outAnimation
    };
    private static int[] touchMarginIds = new int[]{
            R.styleable.ButtonX_carbonX_touchMargin,
            R.styleable.ButtonX_carbonX_touchMarginLeft,
            R.styleable.ButtonX_carbonX_touchMarginTop,
            R.styleable.ButtonX_carbonX_touchMarginRight,
            R.styleable.ButtonX_carbonX_touchMarginBottom
    };
    private static int[] tintIds = new int[]{
            R.styleable.ButtonX_carbonX_tint,
            R.styleable.ButtonX_carbonX_tintMode,
            R.styleable.ButtonX_carbonX_backgroundTint,
            R.styleable.ButtonX_carbonX_backgroundTintMode,
            R.styleable.ButtonX_carbonX_animateColorChanges
    };

    private void initButton(AttributeSet attrs, int defStyleAttr) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.ButtonX, defStyleAttr, R.style.carbonX_Button);

        int ap = a.getResourceId(R.styleable.ButtonX_android_textAppearance, -1);
        if (ap != -1)
            setTextAppearanceInternal(ap);

        for (int i = 0; i < a.getIndexCount(); i++) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.ButtonX_carbonX_textAllCaps) {
                Log.i("setAllCaps: ", "true");
                setAllCaps(a.getBoolean(R.styleable.ButtonX_carbonX_textAllCaps, true));
            } else if (!isInEditMode() && attr == R.styleable.ButtonX_carbonX_fontPath) {
                String path = a.getString(attr);
                Typeface typeface = TypefaceUtils.getTypeface(getContext(), path);
                setTypeface(typeface);
            } else if (attr == R.styleable.ButtonX_carbonX_fontFamily) {
                int textStyle = a.getInt(R.styleable.ButtonX_android_textStyle, 0);
                Typeface typeface = TypefaceUtils.getTypeface(getContext(), a.getString(attr), textStyle);
                setTypeface(typeface);
            }
        }

        CarbonX.initRippleDrawable(this, a, rippleIds);
        CarbonX.initTint(this, a, tintIds);
        CarbonX.initAnimations(this, a, animationIds);
        CarbonX.initTouchMargin(this, a, touchMarginIds);
        CarbonX.initElevation(this, a, R.styleable.ButtonX_carbonX_elevation);

        setCornerRadius((int) a.getDimension(R.styleable.ButtonX_carbonX_cornerRadius, 0));
    ...
    private void setTextAppearanceInternal(int resid) {
        ...
        if (appearance != null) {
            for (int i = 0; i < appearance.getIndexCount(); i++) {
                ...
                if (attr == R.styleable.TextAppearance_carbonX_textAllCaps) {
                    setAllCaps(appearance.getBoolean(R.styleable.TextAppearance_carbonX_textAllCaps, true));
                } else if (!isInEditMode() && attr == R.styleable.TextAppearance_carbonX_fontPath) {
                    String path = appearance.getString(attr);
                    Typeface typeface = TypefaceUtils.getTypeface(getContext(), path);
                    setTypeface(typeface);
                } else if (attr == R.styleable.TextAppearance_carbonX_fontFamily) {
    ...
    android.animation.ValueAnimator.AnimatorUpdateListener tintAnimatorListener = animation -> {
        updateTint();
        ViewCompat.postInvalidateOnAnimation(Button.this);
    };
    android.animation.ValueAnimator.AnimatorUpdateListener backgroundTintAnimatorListener = animation -> {
        updateBackgroundTint();
        ViewCompat.postInvalidateOnAnimation(Button.this);
    };
    android.animation.ValueAnimator.AnimatorUpdateListener textColorAnimatorListener = animation -> setHintTextColor(getHintTextColors());
