...
public class RippleDrawableFroyo extends LayerDrawable implements RippleDrawable {
    ...
    public RippleDrawableFroyo(@NonNull ColorStateList color, @Nullable Drawable content,
        ...
        if (mask != null) {
            addLayer(mask, null, R.id.carbonX_mask, 0, 0, 0, 0);
        }
    ...
    public void inflate(Resources r, XmlPullParser parser, AttributeSet attrs, Resources.Theme theme)
        ...
        final TypedArray a = obtainAttributes(r, theme, attrs, R.styleable.RippleDrawableX);
    ...
    private void updateStateFromTypedArray(TypedArray a) throws XmlPullParserException {
        ...
        final ColorStateList color = a.getColorStateList(R.styleable.RippleDrawableX_android_color);
        if (color != null) {
            mState.mColor = color;
        }

        mState.mMaxRadius = a.getDimensionPixelSize(
                R.styleable.RippleDrawableX_android_radius, mState.mMaxRadius);
    ...
    private void verifyRequiredAttributes(TypedArray a) throws XmlPullParserException {
        if (mState.mColor == null && (mState.mTouchThemeAttrs == null
                || mState.mTouchThemeAttrs[R.styleable.RippleDrawableX_android_color] == 0)) {
    ...
    public void getOutline(@NonNull Outline outline) {
        ...
        for (int i = 0; i < N; i++) {
            if (children[i].mId != R.id.carbonX_mask) {
    ...
    private void drawContent(Canvas canvas) {
        ...
        final int count = mLayerState.mNum;
        for (int i = 0; i < count; i++) {
            if (array[i].mId != R.id.carbonX_mask) {
    ...
    public Drawable mutate() {
        ...
        mMask = findDrawableByLayerId(R.id.carbonX_mask);
